...
export function SetupCalculator({ initialBalance = 10000 }: { initialBalance?: number }) {
    ...
    <Card className="w-full h-full bg-background/50 border-border">
      ...
      <CardContent className="space-y-6">
        <div className="space-y-4">
          ...
          <div className="grid gap-2">
            <div className="flex justify-between">
              ...
              <span className="text-sm text-foreground font-medium">{riskPercent}%</span>
            </div>
            <Slider
              value={[riskPercent]}
        ...
        <div className="pt-4 border-t border-border space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-muted/30 p-3 rounded-lg">
                        ...
                        <p className="text-lg font-bold text-destructive">
                             {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(results.riskAmount)}
                        </p>
                    </div>
                    <div className="bg-muted/30 p-3 rounded-lg">
                         <p className="text-xs text-muted-foreground">Position Size</p>
                         <p className="text-lg font-bold text-primary">
                             {results.positionSize.toFixed(4)} <span className="text-xs font-normal text-muted-foreground">Units</span>
                         </p>
                    </div>
                    <div className="col-span-2 bg-muted/30 p-3 rounded-lg flex justify-between items-center">
                         <div className="text-left">
                            ...
                            <p className={`text-lg font-bold ${results.rrRatio && results.rrRatio >= 2 ? 'text-green-500' : 'text-foreground'}`}>
                                {results.rrRatio ? `1:${results.rrRatio.toFixed(2)}` : '-'}
                            </p>
                         </div>
                         {results.rrRatio && results.rrRatio >= 2 && (
                             <span className="text-xs bg-green-500/10 text-green-500 px-2 py-1 rounded">Good Setup</span>
                         )}
                    </div>
                </div>
            ) : (
           ...
           <Button variant="ghost" size="sm" onClick={clear} className="w-full text-muted-foreground hover:text-foreground">
